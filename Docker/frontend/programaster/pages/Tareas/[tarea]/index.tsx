...
function Home() {
  ...
  const baseUsEjTarURL = "http://127.0.0.1:8000/my_view/usuarioEjercicioTarea/";
  ...
  useEffect(() => {
    async function fetchData(){
      const data = await axios.get(`http://127.0.0.1:8000/my_view/ejercicioTarea/?tarea_id=${router.query.tarea as string}`)
      const question = await axios.get(`http://127.0.0.1:8000/my_view/tarea/?tarea_id=${router.query.tarea as string}`)
  ...
  const runCode = (see: any, id_ejercicio: number) => {
    ...
    const submit = false
    axios.post('http://127.0.0.1:8000/my_view/code/', { code, submit, tests }).then(({ data }) => {
